...
Let's consider this output piece by piece. First, the formula usused to describe 
...
runs = -2789.2429 + 0.6305at_bats
runs = -2789.2429 + 0.6305*5579
runs = 728.3166
For 5579 at-bats model would predict 728 runs.
...
713-728.3166 
```
The model overestimates for this value. Residual is -15.32
...
false statement: The residuals show a curved pattern. 
...
We chose to take the "Hits" variable as the explanatory variable.
```{r runs-vs-sth-else}
ggplot(mlb11, aes(x=hits, y=runs))+ geom_point(shape =1) + geom_smooth(method=lm)
mlb11 %>% summarise(cor(runs, hits))

```
runs  =  -375.56 + 0.7589 hits
R = 80%
R²=64%
There seem to be a strong linear relationship between the 2 variable (runs, hits)
...
R² (runs, at_bats) = 37%
R² (runs, hits) = 64%
The "Hits" model predicts 64% of variability of the runs explained by hits
...
```{r runs-vs-others}
ggplot(mlb11, aes(x=bat_avg, y=runs))+ geom_point(shape =1) + geom_smooth(method = lm)
mlb11 %>% summarise(cor(runs, bat_avg))
m4 <- lm(runs ~ bat_avg, data=mlb11)
summary(m4)

ggplot(mlb11, aes(x=wins, y=runs))+ geom_point(shape =1) + geom_smooth()
mlb11 %>% summarise(cor(runs, wins))
m5 <- lm(runs ~ wins, data=mlb11)
summary(m5)

```
List of variable ordered by prediction efficiency (with linear modeling):
1. batting average (R = 81%, R² = 65%)
2. hits (R = 80%, R² = 64%)
3. at_bats (R = 61%, R² = 37%)
4. wins (R = 60%, R² = 36%)
...
```{r runs-vs-new-vars}
ggplot(mlb11, aes(x=new_obs, y=runs))+ geom_point(shape =1) + geom_smooth(method = lm)
mlb11 %>% summarise(cor(runs, new_obs))
m7 <- lm(runs ~ new_obs, data=mlb11)
summary(m7)

ggplot(mlb11, aes(x=new_slug, y=runs))+ geom_point(shape =1) + geom_smooth(method = lm)
mlb11 %>% summarise(cor(runs, new_slug))
m8 <- lm(runs ~ new_slug, data=mlb11)
summary(m8)

ggplot(mlb11, aes(x=new_onbase, y=runs))+ geom_point(shape =1) + geom_smooth(method = lm)
mlb11 %>% summarise(cor(runs, new_onbase))
m9 <- lm(runs ~ new_onbase, data=mlb11)
summary(m9)
```

1. on-base plus slugging: R= 96.69%  R² = 93%
2. slugging percentage: R= 94.70% R² = 93.49%
3. on-base percentage : R= 92.14% R²= 84.91%
...
```
All principles are verified (linearity, nearly linear distribution of residuals, nearly constant variability)
<div id="license">
