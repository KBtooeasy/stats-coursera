...
hist(evals$score)
```
False statement: The left skewness of the data suggests that the students are less likely to rate the professors highly.
...
We chose to analyse relationship between "age" and "bty_avg" (average beauty)
```{r two-vars-eda}
# Create a scatterplot for 'age' vs 'bty_avg':
ggplot(data= evals, aes(x= age,y= bty_avg )) + geom_point()
# Create a side-by-side boxplots for beauty average vs gender:
...
Is anything awry?
Seems there are more observations than actual points on the scatterplot (overplotting).
...
```{r summary-model}
#fitting linear model for score (response) and average beauty (explanatory)
lm_score_bty = lm(score ~ bty_avg, data= evals)
summary(lm_score_bty)
```
score = 3.88034 + 0.06664bty_avg
...
True ! It's statistically significant because p-value is really low, butmay not be pratically significant since the slope is very small (0.067)
...
```
Incorrect: Nearly normal residuals: Residuals are right skewed, but the sample size is large, so this may not be an important violation of conditions. 
...
summary(m_bty_gen)
```
Yes the slope for bty_avg has changed after adding gender into the model. 
...
True
According to our mode: score = 3.74734 + 0.07416bty_avg + 0.17239gendermale
in case of a woman: score = 3.74734 + 0.07416bty_avg 
in case of a man : score = 3.74734 + 0.07416bty_avg + 0.17239
So, with the same beauty, the model predicts a better score for a male professor
...
I would say that the picture color or number of credits should have the highest p-value. 
...
```
If you look at the summary, you see that the coefficients and \(p\)-values of some variables changed. 
One of the things that makes multiple regression interesting is that coefficient estimates depend on the other variables that are included in the model.
...
```
Adjusted R² is lower than the full model. 
...
Adjusted R² is even lower than the previous one and lower than the full model. 
...
Yes we would need to add the course in the model
...
Male, not old, from an english country, not in minority, teaching a one credit class, and ranked as "professor".
```{r adj-rsq-select}
#Example: 
...
Not, because there was no random sampling over whole univ.
